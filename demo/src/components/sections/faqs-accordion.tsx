  ...
  return (
    <div id={id} {...props}>
      <button
        ...
        className="flex w-full items-start justify-between gap-6 py-4 text-left text-base/7 text-honey-950 dark:text-white"
      >
        {question}
      ...
      <ElDisclosure
        id={`${id}-answer`}
        hidden
        className="-mt-2 flex flex-col gap-2 pr-12 pb-4 text-sm/7 text-honey-700 dark:text-honey-400"
      >
        {answer}
      </ElDisclosure>
    </div>
  ...
  return (
    <section className={clsx('py-16', className)} {...props}>
      <div className="mx-auto flex max-w-3xl flex-col gap-6 px-6 lg:max-w-5xl lg:px-10">
        ...
        <div className="divide-y divide-honey-950/10 border-y border-honey-950/10 dark:divide-white/10 dark:border-white/10">
          {children}
        </div>
      </div>
    </section>

...
export function FooterLink({ href, className, ...props }: { href: string } & Omit<ComponentProps<'a'>, 'href'>) {
  return (
    <li className={clsx('text-honey-700 dark:text-honey-400', className)}>
...
export function SocialLink({
  ...
  return (
    <Link
      ...
      target="_blank"
      aria-label={name}
      className={clsx('text-honey-950 *:size-6 dark:text-white', className)}
      {...props}
    />
  )
}
...
export function NewsletterForm({
  ...
  return (
    <form className={clsx('flex max-w-sm flex-col gap-2', className)} {...props}>
      <p>{headline}</p>
      <div className="flex flex-col gap-4 text-honey-700 dark:text-honey-400">{subheadline}</div>
      <div className="flex items-center border-b border-honey-950/20 py-2 has-[input:focus]:border-honey-950 dark:border-white/20 dark:has-[input:focus]:border-white">
        <input
          type="email"
          placeholder="Email"
          aria-label="Email"
          className="flex-1 text-honey-950 focus:outline-hidden dark:text-white"
        />
        <button
          type="submit"
          aria-label="Subscribe"
          className="relative inline-flex size-7 items-center justify-center rounded-full after:absolute after:-inset-2 hover:bg-honey-950/10 dark:hover:bg-white/10 after:pointer-fine:hidden"
        >
...
export function FooterWithNewsletterFormCategoriesAndSocialIcons({
  ...
  return (
    <footer className={clsx('pt-16', className)} {...props}>
      <div className="bg-honey-950/2.5 py-16 text-honey-950 dark:bg-white/5 dark:text-white">
        <Container className="flex flex-col gap-16">
          ...
          <div className="flex items-center justify-between gap-10 text-sm/7">
            <div className="text-honey-600 dark:text-honey-500">{fineprint}</div>

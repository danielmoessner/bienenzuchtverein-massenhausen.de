...
export function BrandCard({
  ...
  return (
    <div
      className={clsx('flex flex-col justify-between gap-6 rounded-xl bg-honey-950/2.5 p-6 dark:bg-white/5', className)}
      {...props}
    >
      <div className="flex flex-col items-start gap-2">
        <div className="flex h-8 shrink-0">{logo}</div>
        <p className="text-sm/7 text-honey-700 dark:text-honey-400">{text}</p>
      </div>
      <p className="text-xs/6 text-honey-700 dark:text-honey-400">{footnote}</p>
    </div>
  )
}
